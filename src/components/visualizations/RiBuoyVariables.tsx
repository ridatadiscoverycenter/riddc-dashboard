...
export default function RiBuoyVariables({ data, colors = BASE_COLORS, height = 300, width = 600 }: RiBuoyVariablesProps) {
  //console.log({ RiBuoyVariables: "component", data });
  const stations = React.useMemo(() => Array.from(new Set(data.map((value) => value.stationName))), [data]);
  const variables = React.useMemo(() => Array.from(new Set(data.map((value) => value.variable))), [data]);
  const colorsUsed = React.useMemo(() => colors.sort((_, __) => (Math.random() * 2) - 1).slice(0, stations.length), [colors, stations]);
  const buoyVariablesSpec = React.useMemo<VisualizationSpec>(
    ...
    data: [
        ...
        },
        {
            name: "filtered",
            source: "data",
            transform: [{type: "formula", as: "value", expr: "toNumber(datum.value)"}, {type: "filter", expr: "datum.value !== null"}]
        },
        {
    ...
    scales: [
      ...
      },
      {
        name: "variableScale",
        type: "ordinal",
        range: [[1, 0], [1, 1], [3, 1], [5, 2]].slice(0, variables.length),
        domain: variables,
      },
    ],
    legends: [
