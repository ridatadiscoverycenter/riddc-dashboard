...
export function WaterTempChart({ data }: { data: Temperature[] }) {
  const graphData = movingAvg(data, 20);

  const { dates, datasets } = React.useMemo(() => {
    const sortedData = graphData.sort(({ timestamp: time1 }, { timestamp: time2 }) =>
  ...
  const dataGroups = React.useMemo(() => {
    ...
    return group.map(([key, data]) => {
      ...
      const dataWithBlanks = Array.from(Array(dates.length), (_, i) => dates[i]).map(
        (date) => data.find(({ timestamp }) => timestamp.valueOf() === date.valueOf())?.delta
      );
      return {
        label: key,
        data: dataWithBlanks,
