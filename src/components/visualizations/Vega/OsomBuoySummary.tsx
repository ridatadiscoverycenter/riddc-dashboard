...
export function OsomBuoySummary({ data }: OsomBuoySummaryProps) {
  const size = useScreenSize();
  const options = OSOM_VARIABLES.map((key) => ({
    label: variableToLabel(key, 'osom'),
    value: key,
  }));
  const [variable, setVariable] = React.useState<{ value: OsomBuoyVariable }>(options[0]);
    ...
    <>
      <form className="self-stretch">
        <Select
          ...
          onChange={(newValue) =>
            setVariable(newValue as { value: OsomBuoyVariable; label: string })
          }
          options={options}
          dataset="osom"
        />
