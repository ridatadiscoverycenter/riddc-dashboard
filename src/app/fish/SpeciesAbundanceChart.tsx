...
import type { Species, Sample } from '@/utils/erddap/db';

type SpeciesAbundanceChartProps = {
  data: Sample[];
  species: Species;
};

function makeSpeciesAbundanceSpec(colors: ReturnType<typeof useColorMap>): VisualizationSpec {
  return {
    ...
    data: [
      {
        name: 'table',
        //values: prop//s.dataset,
        transform: [{ type: 'formula', as: 'time', expr: 'utc(datum.year, 0)' }],
      },
    ],
    scales: [
      {
      ...
      },
      {
        name: 'color',
        type: 'ordinal',
        range: colors.map(({ color }) => color),
        domain: colors.map(({ id }) => id),
      },
      {
        name: 'shape',
        type: 'ordinal',
...
export default function SpeciesAbundanceChart({ data, species }: SpeciesAbundanceChartProps) {
  const filteredSamples = React.useMemo(
    () => data.filter((sample) => sample.species === species),
    [data, species]
  );
  const colors = useColorMap(['Fox Island', 'Whale Rock']);
  const spec = React.useMemo(() => makeSpeciesAbundanceSpec(colors), [colors]);
  console.log({ filteredSamples, colors, spec });
  return <VegaLite spec={spec} data={{ table: filteredSamples }} />;
}
